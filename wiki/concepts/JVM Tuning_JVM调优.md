...
JVM 调优是指通过调整 JVM 启动参数、选择合适的垃圾收集器（GC），以及利用诊断工具排查内存或 CPU 问题，从而让运行在 [[JVM Run-Time Data Areas_JVM 运行时数据区|JVM 运行时数据区]] 的 Java 应用达到吞吐量、延迟和内存占用的最佳平衡。
...
## 相关链接
- 内存结构基础：[[JVM Run-Time Data Areas_JVM 运行时数据区|JVM 运行时数据区]]